...
class TestIterEnrichment:
    def create_test_files(self):
        ...
        return BackgroundGeneSet(str(ROOT / "tmp" / "background.txt")), GeneSetLibrary(
            str(ROOT / "tmp" / "test_lib.gmt")
        )

    def test_iterative_enrichment_peels_off(self):
        background_gene_set, gene_set_library = self.create_test_files()
        genes = GeneSet(["A", "B", "C"], {"A", "B", "C"})
